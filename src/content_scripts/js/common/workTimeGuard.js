...
;(async function () {
    ...
    // 检测当前时间是否为工作时间
    function isInWorkTime(begin = 8, end = 22) {
        // 开发阶段，暂时先设定为8点到22点
        const now = new Date()
                ...
                this.timeoutId = setTimeout(
                    () => {
                        ...
                        tabManager.closeAfter(5, {
                            handleTick: () => {
                                this.setCloseTips(
                                    `This page will be closed in ${tabManager.count} seconds.`
                                )
                            },
                        })
                    },
    ...
    if (IS_ENABLE && isInWorkTime()) {
        ...
        if (lastLeisureTime) {
            ...
            if (diff < 1000 * 60 * LEISURE_MINUTES) {
                console.log(
                    `距离上次允许使用时间小于${LEISURE_MINUTES}分钟，不显示遮罩`
                )
                console.log(
                    `剩余${LEISURE_MINUTES * 60 - Math.floor(diff / 1000)}秒可浏览`
                )
                // 开启定时
                setTimeout(
                    () => {
        ...
        function main() {
            ...
            tabManager.closeAfter(5, {
                ...
                afterClose: () => {
                    console.log('关闭页面失败')
                    pageMask.setCloseTips(
                        'Failed to auto close the page, please close it manually.'
                    )
