...
if (isUrlIncluded('https://leetcode.cn/problems')) {// 题目页面
    // document.querySelector('.monaco-editor').classList.contains('focused')
    waitForElement('.relative.flex.flex-1.items-center.justify-end').then((container) => {
        let [status, button] = getStatus(container);
        if (status === 'init') { // 初始状态，自动开始计时
            button.parentElement.parentElement.click();
            waitForElement('[role=popover] svg.fa-play').then((element) => {
                let startButton = element.parentElement.parentElement.parentElement;
                startButton.click();
                toast('已自动开始计时');
            })
        } else if (status === 'running') {// // 已经在计时了，提示正在计时即可
            toast('正在计时');
        } else if (status === 'paused') {// 暂停中，提示恢复计时
            toast('已暂停，请恢复计时');
        }
